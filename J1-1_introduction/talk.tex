...
\begin{frame}
  ...
  \only<1>{Four days
    \begin{itemize}
    \item Friday 9 December \gray{(9h)}
    \item Friday 16 December \gray{(9h)}
    \item Friday 13 January \gray{(8h30)}
    \item Thursday 9 March \gray{(8h30)}
    \end{itemize}
  }
  \only<2>{
    Communication
    \begin{itemize}
    \item \url{https://ynov-ml.slack.com/}
    \item \url{https://github.com/ynov-ml/2016-2017}
    \end{itemize}
  }
  \only<3>{Tasks
    \begin{itemize}
    \item Email \url{jeff@purple.com} so I can invite you to slack.
    \item Create a github account if you don't have one.  Anonymous if you must.
    \item Clone the course repository, add your github repository to \url{repositories}, send a PR.
    \end{itemize}
  }
  \only<4>{Good git practices
    \begin{itemize}
    \item One line summary, declarative, $<60$ characters
    \item Good comments
    \item Work on branches
    \end{itemize}
  }
\end{frame}
